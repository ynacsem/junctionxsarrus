...
import { useClient, useClients } from "@/hooks/use-api";
...
export default function EditClientPage() {
  ...
  const { updateClient, loading: updating, error: updateError } = useClients();
  ...
  // Populate form when client is loaded
  useEffect(() => {
    if (client) {
      setFormData({
        client_id: client.client_id,
        preferred_location: client.preferred_location,
        min_budget_DZD: client.min_budget_DZD.toString(),
        max_budget_DZD: client.max_budget_DZD.toString(),
        min_area: client.min_area.toString(),
        max_area: client.max_area.toString(),
        preferred_property_type: client.preferred_property_type,
        marital_status: client.marital_status,
        has_kids: client.has_kids.toString(),
        weight_location: client.weight_location?.toString() || "",
        weight_property_type: client.weight_property_type?.toString() || "",
        preferred_rooms: client.preferred_rooms?.toString() || "",
        weight_rooms: client.weight_rooms?.toString() || "",
        preferred_schools_nearby: client.preferred_schools_nearby?.toString() || "",
        weight_schools_nearby: client.weight_schools_nearby?.toString() || "",
        preferred_hospitals_nearby: client.preferred_hospitals_nearby?.toString() || "",
        weight_hospitals_nearby: client.weight_hospitals_nearby?.toString() || "",
        preferred_parks_nearby: client.preferred_parks_nearby?.toString() || "",
        weight_parks_nearby: client.weight_parks_nearby?.toString() || "",
        preferred_public_transport_score: client.preferred_public_transport_score?.toString() || "",
        weight_public_transport_score: client.weight_public_transport_score?.toString() || "",
      });
    }
  }, [client]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload: any = {
        ...
        min_budget_DZD: parseFloat(formData.min_budget_DZD),
        max_budget_DZD: parseFloat(formData.max_budget_DZD),
        min_area: parseInt(formData.min_area, 10),
        max_area: parseInt(formData.max_area, 10),
      ...
      const integerFields = [
        "preferred_rooms", "preferred_schools_nearby", "preferred_hospitals_nearby",
        "preferred_parks_nearby", "preferred_public_transport_score"
      ];
      const floatFields = [
        "weight_location", "weight_property_type", "weight_rooms",
        "weight_schools_nearby", "weight_hospitals_nearby", "weight_parks_nearby", "weight_public_transport_score"
      ];
      ...
      await updateClient(Number(clientId), payload);
              ...
              <CardContent className="space-y-4">
                <div>
                  ...
                  <Input
                    ...
                    readOnly
                  />
                  <p className="text-xs text-muted-foreground mt-1">Client ID cannot be changed</p>
                  {errors.client_id && (
